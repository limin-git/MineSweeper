...
{	// 搜索是雷的可能性最小的随机位置
	...
	set< Point > Set_Backup_Point;
	// 遍历雷区获取候补位置集合
	for (int i = 1; i <= m.row; ++i) {
		for (int j = 1; j <= m.col; ++j) {
			if (m.field[i][j].appear == MASKED) { // 是未知格
				if (m.cnt_num_near(i, j) == 0) continue;
				double posb = 0;	
				...
				if (0 < posb && posb < WHOLE_POSSIBLITY) { // 如果该格是雷的可能比全局是雷的可能性小则加入到集合中
...
void main()
{
	srand(time((time_t*) NULL));			// 随机种子要放在这里
	int cnt = 1;
	int S = 0, F = 0;
	set< S_M_F > Set_Of_Sub_Mine_Field;	// 子雷区集合
	S_O_P Set_Mine, Set_Nomine;			// 雷和非雷位置集
	...
	while (cnt <= 1000) {
		...
		if (state == FAILED) {
			cout << "第 " << cnt++ << " 次扫雷失败 " << endl;
			++F;
		}
		else {
			cout << "第 " << cnt++ << " 次扫雷成功 " << endl;
			++S;
		}
	}
	system ("cls");
	cout << "正确率" << S / 10.0 << " % " << endl;
}
