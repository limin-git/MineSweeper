...
typedef double POSB;				// 是雷的可能性possibility
set< Point > Set_Backup_Point;		// 候补位置集合, 以POSB升序排序
...
{	// 将雷区内边界划分成子雷区集合
	...
	for (int i = 1; i <= m.row; i++) {
		for (int j = 1; j <= m.col; j++) {
			if (m.field[i][j].is_number()) { // 在雷区外边界搜未知格
				sop.clear();
				for (int k = 0; k < 8; k++) {	// 查看四周
					p = Point(i, j) + OFFSET[k];
					if (m.field[p.getx()][p.gety()].appear == MASKED) { // 是未展开雷区的边界
						sop.insert(p);			// 加入位置集合
					}
				}
				if (sop.size() > 0) {			// 找到一个子雷区
					S_M_F f(sop, m.field[i][j].appear - m.cnt_marked_near(i, j));	// 构造子雷区
...
STATE_GAME game_state(const MineField& m)
{
	...
	for (int i = 1; i <= m.row; ++i) {
		for (int j = 1; j <= m.col; ++j) {
			if (m.field[i][j].appear == MASKED) Flag = false;
			if (m.field[i][j].appear == BLAST || m.field[i][j].appear == MINE) return FAILED;
		}
	}
	if (Flag) 
		return SUCCESS;
...
{	// 搜索是雷的可能性最小的随机位置
	double WHOLE_POSSIBLITY = (1.0 * m.num) / (m.row * m.col); // 全局是雷的可能性
	multimap < double, Point > Map_Backup_Point;
	// 遍历雷区获取候补位置集合
	for (int i = 1; i <= m.row; ++i) {
		for (int j = 1; j <= m.col; ++j) {
			if (m.field[i][j].appear == MASKED) { // 是未知格
				if (m.cnt_num_near(i, j) == 0) {
					Map_Backup_Point.insert(pair<double, Point>(WHOLE_POSSIBLITY, Point(i, j)));
					continue;
				}
				double posb = 0;	
				for (int k = 0; k < 8; ++k) { // 查看该位置是否属于子雷区, 即周围是否有数字格
					if (m.field[i + OFFSET[k].getx()][j + OFFSET[k].gety()].is_number()) {
						double c = m.cnt_masked_near(i + OFFSET[k].getx(), j + OFFSET[k].gety());
						double tp = 0; 
						if (0 < c) 
							tp = m.field[i + OFFSET[k].getx()][j + OFFSET[k].gety()].appear / c;
						if (posb < tp) posb = tp;
					}
				}
...
void main()
{
	...
	while (true) {
		CNT_GUSS = 0;
		long t = clock();
		MineField m(16, 30, 99);			// 这里定义等级
		set< S_M_F > Set_Of_Sub_Mine_Field;	// 子雷区集合
		S_O_P Set_Mine, Set_Nomine;			// 雷和非雷位置集
		list < CMD > List_Of_Command;		// 命令列表
		STATE_GAME state;
		while ((state = game_state(m)) == CONTINUE) {
			// 初始化变量 
		...
		cout	<< STATE_INFOR[state] << endl 
				<< "本次共进行了 " << CNT_GUSS << " 次猜测" << endl
				<< "used " << (clock() - t) / 1000.0 << " seconds" << endl;
		system("pause");
	}
}
